...
const PopularTag = ({item}) => {
    return(
        <>
            <li className=' cursor-pointer px-2 py-2 bg-dark-button rounded-lg font-normal text-black'>
                {item.tag}
            </li>
        </>
    )
...
const Hero = () => {
  return (
    ...
    <p className='text-dark-text font-bold lg:text-2xl text-xl text-center'>Your Journey to Knowledge Starts Here</p>
        <div className='xl:flex xl:justify-start xl:items-start xl:px-0 flex flex-col items-center gap-5 lg:flex lg:flex-row lg:px-56   rounded-lg lg:py-3 lg:items-center  lg:justify-left  lg:gap-4'>
            <input placeholder="Search" className='py-5 text-center w-80  shadow-[0_8px_60px_rgb(0,0,0,0.12)] border-dark-hard h-8 lg:py-5  outline-none lg:px-8  rounded-lg' type="text" />
        <button className='search text-center flex items-center py-5 h-8 px-20 shadow-[0_8px_60px_rgb(0,0,0,0.12)]
         bg-dark-button   text-black font-normal lg:py-5 lg:px-6 rounded-lg
         '>
            Search
        </button>
        </div>
